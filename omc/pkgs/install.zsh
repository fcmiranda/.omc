...
# Script to install packages: kanshi, atuin, ghostty, keyd, stow, and yazi
...
# Install stow
if [ -f "$(dirname "$0")/stow.zsh" ]; then
    zsh "$(dirname "$0")/stow.zsh"
else
    echo "stow.zsh not found, skipping stow installation"
fi

# Install yazi
if [ -f "$(dirname "$0")/yazi.zsh" ]; then
    zsh "$(dirname "$0")/yazi.zsh"
else
    echo "yazi.zsh not found, skipping yazi installation"
fi
